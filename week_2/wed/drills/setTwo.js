...
//['Tyler', 'Cahlan', 'Ryan', 'Colt', 'Tyler', 'Blaine', 'Cahlan']
const uniq = (uniqArr, cb) => {
  cb([...(new Set(uniqArr))])
}

uniq(names,result => {
  console.log(`The new array with the duplicate items removed is ${result}`)
});
...
const each = (arr,cb) => {
  for(let i = 0; i< arr.length; i++){
    cb(`The item at index ${i} is ${arr[i]}.`)
  }
}
...
const addFortyThree= addingFactory(43)

console.log(addFortyThree(45))
// ////////// CHALLENGE 2 //////////
...
const getUserById = (arr,identifier,cb) =>{
  for(let i = 0; i < arr.length; i++){
