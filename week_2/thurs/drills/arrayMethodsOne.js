...
// CODE HERE
//by default, because there was not an initial value declared, it will use the first element in
//the array as the accumulator and add it to the cv which will be the second element
...
// CODE HERE
//the filter(takes the element with the property CP and checks and filters out the elements over 200)
...
// CODE HERE
const newOrders = orders.map(element => (element.price * (1+element.tax)))
...
const identifier = arr => {
    return purchases.filter(el => el.owner === 'Bob').reduce((acc, cv) => acc + cv.price,0);
}
console.log(identifier(purchases))