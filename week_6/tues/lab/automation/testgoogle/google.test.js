...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("rugs", Key.RETURN);
  ...
  await driver.findElement(By.name("q")).sendKeys("phone",Key.RETURN)
  // TODO Wait for the results page to load
  const currentPage = await driver.wait(until.titleIs("phone - Google Search"),2000)
});
