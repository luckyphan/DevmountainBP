...
/*
    ...
    and its text color to white (you can use the .style object or create a 
...
sayHelloButton.addEventListener('mouseover',hello)
...
/*
    ...
    Write another function that changes the button back to its original colors. #EFEFEF for the background and black 
    for the text.
...
sayHelloButton.addEventListener('mouseover',backHello)
...
/*
    ...
    Inside the callback function, console.log the response's data (in the intermediate instructions 
        we'll come back to this function and add HTML).
         ...
         Back in the ohMy function on Problem 5, replace the console log in the promise's callback with a for loop 
    that loops over res.data. 

    On each iteration of the loop, create a new p element. Set its textContent equal the string at the current 
    index (i) and then append the new p element onto the document's body. 
*/ 
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((res) => {
        ...
        p.appendChild(node);
        const currentP = document.getElementById("repeat-text");
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${'itzapizza'}`).then((res) => {
